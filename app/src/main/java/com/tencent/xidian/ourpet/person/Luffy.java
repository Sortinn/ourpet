...
//一般数据
        ...
        bmpW = decodeResource(res, R.raw.luffy_stand).getWidth();
        bmpH = decodeResource(res, R.raw.luffy_stand).getHeight();
        personSize = Float.parseFloat(sp.getString("person_size", "0.4"));
...
//初始化的xy
    ...
    private void actionTime(Canvas canvas, Paint paint){
        paint.setTextSize(80f * personSize);
    ...
    private void onActionChange(int actionFlag){
        ...
        switch (actionFlag){
            case FLAG_CRAWL:
                bmpLuffy[0] = decodeResource(res, R.raw.luffy_crawl_1);
                bmpLuffy[1] = decodeResource(res, R.raw.luffy_crawl_2);
                break;
            case FLAG_DOWN:
                if(x < 0) x = 0;
                if(x > screenW-bmpW*personSize) x = screenW-bmpW*personSize;
                bmpLuffy[0] = decodeResource(res, R.raw.luffy_down_2);
                bmpLuffy[1] = decodeResource(res, R.raw.luffy_down_3);
                break;
            case FLAG_EAT:
                bmpLuffy[0] = decodeResource(res, R.raw.luffy_eat_1);
                bmpLuffy[1] = decodeResource(res, R.raw.luffy_eat_2);
                bmpLuffy[2] = decodeResource(res, R.raw.luffy_eat_3);
                break;
            case FLAG_SIT:
                upOrDown = -1;//默认向下看
                bmpLuffy[0] = decodeResource(res, R.raw.luffy_sit_1);
                bmpLuffy[1] = decodeResource(res, R.raw.luffy_sit_2);
                break;
            case FLAG_STAND:
                bmpLuffy[0] = decodeResource(res, R.raw.luffy_stand);
                break;
            case FLAG_UP:
                bmpLuffy[0] = decodeResource(res, R.raw.luffy_stand);
                bmpLuffy[1] = decodeResource(res, R.raw.luffy_up_1);
                bmpLuffy[2] = decodeResource(res, R.raw.luffy_up_2);
                break;
            case FLAG_WALK2:
                upOrDown = Math.random()< 0.5 ? 1:-1;
            case FLAG_WALK:
                bmpLuffy[0] = decodeResource(res, R.raw.luffy_stand);
                bmpLuffy[1] = decodeResource(res, R.raw.luffy_walk_1);
                bmpLuffy[2] = decodeResource(res, R.raw.luffy_walk_2);
                break;
        }
    }
}
