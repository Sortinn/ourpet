...
public class Piggy extends Person {
    /**
     * 图片,动作未超过3个（不知此方法是否有问题？）
     */
//    private Bitmap[] bmpPiggy = new Bitmap[3];
    private Bitmap[] bmpPiggy = new Bitmap[5];
    /**
     * 标识常量
     */
    private final int
...
//初始化的xy
    ...
    private void draw(Canvas canvas, Paint paint) {
        ...
        switch (actionFlag) {
            ...
            case FLAG_DRINK:
                actionDrink(canvas, paint);
                break;
            case FLAG_UP:
                actionUp(canvas, paint);
                break;
            case FLAG_FLY:
                actionFly(canvas, paint);
                break;
            case FLAG_WALK2:
    ...
    private void actionCrawl(Canvas canvas, Paint paint) {
        if (x <= 0 && leftOrRight == 1
            ...
            leftOrRight *= -1;
        canvasDraw(canvas, bmpPiggy[frameFlag / 6 % 2], matrix, paint);
        frameFlag++;
    ...
    private void actionDown(Canvas canvas, Paint paint) {
        if (frameFlag <= 1) {
            canvasDraw(canvas, bmpPiggy[0], matrix, paint);
            int disY = (int) (screenH - bmpH / 2 - bmpH * personSize / 2);
            if (y < disY)
                y += speed * 3;
        } else if (frameFlag <= 3) {
            canvasDraw(canvas, bmpPiggy[1], matrix, paint);
        } else {
            int disY = (int) (screenH - bmpH / 2 - bmpH * personSize / 2);
            if (y > disY)
                y = disY;
     ...
     * @param paint
     */
    private void actionDrink(Canvas canvas, Paint paint) {
        canvasDraw(canvas, bmpPiggy[frameFlag % 3], matrix, paint);
        frameFlag++;
    }
    ...
    private void actionUp(Canvas canvas, Paint paint) {
        ...
        //动作绘制
        canvasDraw(canvas, bmpPiggy[frameFlag * leftOrRight], matrix, paint);
        //记录前一个点
        touchPreX = touchX;
    }
     ...
     * 动作：行走,walk
     */
    private void actionFly(Canvas canvas, Paint paint) {
        if (x <= 0 && leftOrRight == 1
                || x + bmpW * personSize >= screenW && leftOrRight == -1)//碰壁换向
            leftOrRight *= -1;
        ...
        if (frameFlag % 2 == 1)
            ...
            canvasDraw(canvas, bmpPiggy[2], matrix, paint);
    ...
    private void setActionGroup(SharedPreferences sp) {
        ...
        if (sp.getBoolean("piggy_action_drink", false))
            actionGroup.add(FLAG_DRINK);

        if (actionGroup.size() == 0) {
            actionGroup.add(FLAG_FLY);
        }
        randomChange();
    }
